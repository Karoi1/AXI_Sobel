...
def check_ctrl_out(records):
    ...
    if not tuser_recs:
        ...
        # SOF 协议: tuser=1 时 tvalid 必须为 0
        if v != 0:
            errors.append(
                f"SOF 协议错误: tuser=1 时 tvalid={v} (预期 0), time={t}. "
                f"VDMA S2MM 要求 tuser 必须于 tvalid 之前（至少一拍）出现"
            )
        ...
        print(f"  SOF time={t}, tvalid={v} ✓ (同拍协议)")
...
# ============================================================
def main():
    ctrl_in_file = f"{BASE}\\sim_logger\\ctrl_in.txt"
    ctrl_out_file = f"{BASE}\\sim_logger\\ctrl_out.txt"
    pix_file     = f"{BASE}\\sim_logger\\pix_out.txt"
    ...
    # 保存实际输出
    cv2.imwrite(f"{BASE}\\test_imgs\\sim_result\\sobel_actual.png", actual)

    # 生成参考 Sobel
    img_gray = cv2.imread(f"{BASE}\\test_imgs\\sim_result\\img_gray_640x480.png", cv2.IMREAD_GRAYSCALE)
    reference = sobel_reference(img_gray)
    cv2.imwrite(f"{BASE}\\test_imgs\\sim_result\\sobel_reference.png", reference)
    ...
    cv2.imwrite(f"{BASE}\\test_imgs\\sim_result\\sobel_diff.png", diff_img)

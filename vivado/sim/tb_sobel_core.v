...
module tb_sobel_core;
    ...
    initial begin
        $readmemh("E:/vivadoproject/pcam-udp/sim_logger/input.hex", img_rom);
        f_ctrl    = $fopen("E:/vivadoproject/pcam-udp/sim_logger/ctrl_out.txt", "w");
        f_pix     = $fopen("E:/vivadoproject/pcam-udp/sim_logger/pix_out.txt", "w");
        f_ctrl_in = $fopen("E:/vivadoproject/pcam-udp/sim_logger/ctrl_in.txt", "w");
        f_hr      = $fopen("E:/vivadoproject/pcam-udp/sim_logger/hr_frame.txt", "w");
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (beat_cnt < 76800) begin
                if (beat_in_row == 8'd0 && row_cnt > 9'd0 && gap_cnt < 3) begin
                    // 行间隙: 每行结束后 3 拍无效
                    s_axis_tvalid <= 1'd0;
                    s_axis_tlast  <= 1'd0;
                    s_axis_tuser  <= 1'd0;
